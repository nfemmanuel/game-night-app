...
function UnoNMGame() {
    ...
    const [showDiscardAllPicker, setShowDiscardAllPicker] = useState(false);
    const [showRouletteColorPicker, setShowRouletteColorPicker] = useState(false);
    const [rouletteDrawnCards, setRouletteDrawnCards] = useState([]);
    ...
    const [showHandRotationMessage, setShowHandRotationMessage] = useState(false); // PHASE 3
    const [handRotationDirection, setHandRotationDirection] = useState(''); // PHASE 3
    ...
    // PHASE 3: Show hand rotation popup
    const showHandRotation = (direction) => {
        setHandRotationDirection(direction === 1 ? 'clockwise' : 'counter-clockwise');
        setShowHandRotationMessage(true);
        setTimeout(() => {
            setShowHandRotationMessage(false);
        }, 2500);
    };
    ...
    const handleCpuRoulette = (cpu) => {
        const colors = ['red', 'yellow', 'green', 'blue'];
        const randomColor = colors[Math.floor(Math.random() * colors.length)];

        setGameMessage(`${cpu.name} chose ${randomColor} and is drawing...`);

        setTimeout(() => {
            const drawnCards = game.drawUntilColor(cpu, randomColor);
            game.rouletteActive = false;

            setGameMessage(`${cpu.name} drew ${drawnCards.length} card(s) until getting ${randomColor}!`);
    ...
    // PHASE 3: CPU chooses best target for 7 card swap based on difficulty
    const chooseCpuSwapTarget = (cpu) => {
        ...
        if (difficulty === 'easy') {
            ...
            // Medium: Player with most cards
            otherPlayers.sort((a, b) => b.hand.length - a.hand.length);
            return otherPlayers[0];
        } else {
            // Hard: Strategic
            // If CPU has few cards (3 or less), swap with player who has most cards
            // If CPU has many cards, swap with player closest to winning (fewest cards)
            if (cpu.hand.length <= 3) {
                otherPlayers.sort((a, b) => b.hand.length - a.hand.length);
                return otherPlayers[0];
            } else {
                otherPlayers.sort((a, b) => a.hand.length - b.hand.length);
                return otherPlayers[0];
            }
        }
    };
        ...
        setTimeout(() => {
            ...
            if (validCards.length > 0) {
                ...
                if (cardToPlay.value === '7') {
                    // PHASE 3: Use difficulty-based target selection
                    const targetPlayer = chooseCpuSwapTarget(cpu);
                ...
                if (cardToPlay.value === 'discard_all') {
                    ...
                    const discardedCount = game.discardAllColor(cpu, mostCommonColor);

                    setGameMessage(`${cpu.name} discarded ${discardedCount} ${mostCommonColor} card(s)!`);
                    updateGame();
                ...
                // PHASE 3: Check for 0 card before playing
                ...
                // PHASE 3: Show hand rotation popup for 0 card
                if (isZeroCard) {
                    showHandRotation(game.direction);
                }
                ...
                setTimeout(() => {
                    ...
                    if (game.rouletteActive) {
                        ...
                        if (nextPlayer.isHuman && !nextPlayer.isEliminated) {
                            setGameMessage(`${nextPlayer.name}, choose a color to draw until you get it!`);
                            setShowRouletteColorPicker(true);
                            setIsAnimating(false);
                        } else if (!nextPlayer.isEliminated) {
                            handleCpuRoulette(nextPlayer);
                        }
                    } else {
                        setTimeout(() => {
                            setIsAnimating(false);
    ...
    const handleRouletteColorChoice = (color) => {
        setShowRouletteColorPicker(false);
        setGameMessage(`Drawing until you get ${color}...`);

        setTimeout(() => {
            const drawnCards = game.drawUntilColor(humanPlayer, color);
            game.rouletteActive = false;

            setRouletteDrawnCards(drawnCards);
            setGameMessage(`You drew ${drawnCards.length} card(s) until getting ${color}!`);
    ...
    const handlePlayerChoice = (targetIndex) => {
        if (selectedCard) {
            ...
            // PHASE 3: Show swap message
            setGameMessage(`You swapped hands with ${targetPlayer.name}!`);
    ...
    const handleDiscardAllChoice = (color) => {
        if (selectedCard) {
            ...
            const discardedCount = game.discardAllColor(humanPlayer, color);

            setGameMessage(`You discarded ${discardedCount} ${color} card(s)!`);
    ...
    return (
        <div style={{
            ...
            {/* PHASE 3: Hand Rotation Popup */}
            <AnimatePresence>
                {showHandRotationMessage && (
                    <motion.div
                        initial={{ scale: 0, opacity: 0 }}
                        animate={{ scale: 1, opacity: 1 }}
                        exit={{ scale: 0, opacity: 0 }}
                    ...
                    >
                        ...
                        <p style={{
                            ...
                            All hands rotated {handRotationDirection}!
                        </p>
                    </motion.div>
                )}
            </AnimatePresence>
            ...
            {/* PHASE 3: Color Picker with X button */}
            {showColorPicker && (
                <div style={{
                    position: 'fixed',
                    top: 0,
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        <p style={{
                            ...
                            Choose a color - you'll draw cards until you get it!<br />
                        ...
                        <div style={{
                            ...
                            {['red', 'yellow', 'green', 'blue'].map(color => (
                                <motion.button
                                    ...
                                    whileHover={{ scale: 1.1 }}
                                    whileTap={{ scale: 0.95 }}
                                    onClick={() => handleRouletteColorChoice(color)}
                                    style={{
                                        width: isMobile ? '80px' : '120px',
                                        height: isMobile ? '80px' : '120px',
                                        borderRadius: '15px',
