...
function MafiaGame() {
    ...
    const handlePlayerTap = (playerName) => {
        ...
        if (gameState.phase === 'night') {
            switch (gameState.nightStep) {
                ...
                case 'doctor':
                    if (!player.alive && player.killedBy === 'mafia') {
                        const doctor = gameState.getPlayersByRole('doctor')[0];
                        if (player.name === doctor.name && gameState.doctorHasSavedSelf) {
                            // Can't save self twice
                            return;
                        }
                        gameState.doctorRevive(playerName);
                    }
                    break;
    ...
    const getPlayerCardStyle = (player) => {
        ...
        if (!player.alive) {
            ...
            baseStyle.cursor = player.killedBy === 'mafia' && gameState.nightStep === 'doctor' ? 'pointer' : 'not-allowed';
        }
