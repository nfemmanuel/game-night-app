...
class GameState {
    ...
    canPlay(card) {
        if (this.isStacking) {
            return this.isStackableDrawCard(card) && this.getDrawValue(card) >= this.getDrawValue(this.getTopCard());
        }
    ...
    isStackableDrawCard(card) {
        const drawCards = ['draw2', 'wild_draw4', 'wild_draw6', 'wild_draw10'];
        ...
        if (card.card_type === 'action') {
            if (card.value === 'skip') {
                ...
                if (this.players.length > 2) {
                    ...
                    // PHASE 4: In 2-player, reverse acts as skip
                    this.nextTurn();
                }
            }
            ...
            if (card.value === 'wild_draw4') {
                ...
                if (this.players.length === 2) {
                    // PHASE 4: In 2-player, affects current player unless they can stack
                    this.isStacking = true;
                    this.stackedDrawCount = 4;
                    // Don't advance turn - current player must stack or draw
                } else {
                    this.isStacking = true;
                    this.stackedDrawCount = 4;
                    this.nextTurn();
    ...
    discardAllColor(player, color) {
        ...
        // Remove ALL cards of the chosen color
    ...
    // PHASE 4: Fixed checkWinner() for two-player elimination bug
    checkWinner() {
        ...
        // Win condition 2: Last player standing (all others eliminated)
        if (activePlayers.length === 1) {
            return activePlayers[0];
        }

        // PHASE 4 FIX: Check if only one non-eliminated player remains
        // This fixes the two-player elimination bug where winner wasn't detected
        // when one player got eliminated via mercy rule
